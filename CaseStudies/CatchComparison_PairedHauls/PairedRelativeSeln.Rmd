...
nboot=1000
nperm=10 #For bootstrapping and permutation tests, set to >=999 in practice
#source("T:/Russell_SSD/ACTIVE/CurrentWork/RPackages/SELECT/R/BOOTSTRAP&PERMUTATION.R")
```

### Read in the data

The data for each mesh size are in individual `xlsx` sheets and require merging.
...
```{r PH-BootPlotWithPtwiseAndBonf}
#With pointwise confidence band also shown
BootPlot(BootPreds,CLseq,predn,Data=Tots.df,show.bonferroni=T,
...
```{r PH-LimitedBootPlotWithPtwise}
...
```{r PH-OverlayFits, warning=F, message=F}
par(oma=c(0,0,0,0),mar=c(4,4,1,4),las=1)
...
legend("topleft",
       legend=c("Spline","order-weighted quartic","32-submodel quartic"),cex=0.8,
