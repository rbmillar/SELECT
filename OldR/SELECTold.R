...
SELECT=function(Data,dtype,stype="logistic",Q=NULL,Meshsize=NULL,x0=NULL,rel.power=NULL,
                ...
                control=list(maxit=1000,reltol=1e-6),fit=T) {
  ...
  if(is.null(rel.power) | dtype %in% (c("cc","ph"))) rel.power=rep(1,length(Meshsize))
  ...
  fullfit.l=Const+sum(Counts*log(CountPropns),na.rm=TRUE)
  ...
  if(!fit) {
    ...
    z=c(Call=SELECT.call,SELECT.args,loglike.init=ll.init)
    return( inviible(z) ) }
  #Fit=TRUE
  ...
  z=c(Call=SELECT.call,fit,deviance=Dev,SELECT.args,loglike.init=ll.init)
  class(z)="SELECT"
  return(invisible(z))
...
boot.SELECT=function(Data,statistic,R,SetID) {
  z=try( statistic(Data) )
  if(class(z)[1]=="try-error") stop("Error running on actual data")
  cat("Raw data output:",z,"\n")
  BootMatrix=matrix(NA,nrow=R,ncol=length(z))
  cat(paste("Bootstrap data: Starting a",R,"resamples bootstrap...\n"))
  if(is.null(SetID)) stop("SetID (set id variable) is required.")
  PBar <- txtProgressBar(min = 0, max = R, style = 3)
  for(i in 1:R) {
    if(i%%5==0) setTxtProgressBar(PBar, i)
    bootData=Dble.boot(Data,SetID,smooth=F)$bootData
...
nllhood=function(theta,Data,Meshsize,r,rel.power,penalty.func) {
  ...
  rmatrix[is.na(Counts)]=NA #No fitted retention for missing mesh sizes
...
#' @export
ModelCheck=function(fit,minE=0,xlab="Length (cm)",print.out=T,pex=1,...) {
  ...
  if(nmeshes>2&AreLensUnique) {
    ...
    if(nmeshes==2) {
      ...
      plot(lens,Data[,3]/(Data[,2]+Data[,3]),type=ifelse(AreLensUnique,"b","p"),
           ylim=c(0,1),xlab=xlab,ylab="Propn in exptl gear",lab=xyticks,...)
...
#' @export
PlotCurves=function(fit,Meshsize=NULL,rel.power=NULL,plotlens=NULL,standardize=F,
                    xlab="Length (cm)",ylab="Retention curve",...) {
  ...
  colnames(lensmatrix)=c("Length",Meshsize)
